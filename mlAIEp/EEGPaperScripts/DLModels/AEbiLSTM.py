...
def display_metrics(y_true, y_pred):
    ...
    disp.plot()
    conf_name='AE1Confusion.png'                                      # Conf file name
...
def FCN(fileSeizure, fileNoSeizure):
    ...
    from tensorflow.keras.layers import Conv2D, MaxPooling2D, Flatten, Dense, BatchNormalization, Dropout, UpSampling2D, LSTM, Bidirectional
    ...
    # Estandarizar
    scaler_name='AE1Scaler.pkl'                               # Scaler file name 
    sc = StandardScaler()
    ...
    model.add(MaxPooling2D((2, 2), padding='same'))
    model.add(Conv2D(8, (3, 3), activation='relu', padding='same', name='encoder_output'))
    model.add(MaxPooling2D((2, 2), padding='same'))

    # Bidirectional LSTM layer
    model.add(tf.keras.layers.Reshape((138, 28), input_shape=(161, 3, 8)))
    model.add(Bidirectional(LSTM(64, return_sequences=True)))
    model.add(Bidirectional(LSTM(32)))
    model.add(Flatten())
    model.add(tf.keras.layers.Dense(2, activation='softmax'))
    model_filename_CP = 'AE1Grid.h5'
    ...
    model_filename = 'AE1.h5'                                                     # Model File Name
    ...
    plt.ylabel('Loss')
    plt.legend()
    LossEpoch_name='AE1LossEpoch.png'                                      # LossEpoch file name
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    AccEpoch_name='AE1AccEpoch.png'                                      # AccEpoch file name
