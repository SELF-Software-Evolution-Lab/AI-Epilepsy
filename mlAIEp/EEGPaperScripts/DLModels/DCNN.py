...
def display_metrics(y_true, y_pred):
    ...
    disp.plot()
    conf_name='DCNN1Confusion.png'                                      # Conf file name
...
def FCN(fileSeizure, fileNoSeizure):
    ...
    # Estandarizar
    scaler_name='DCNN1Scaler.pkl'                               # Scaler file name 
    sc = StandardScaler()
    ...
    model.add(Conv2D(filters=5, kernel_size=(10, 1), activation='relu', padding='same'))
    model.add(Conv2D(filters=5, kernel_size=(20, 18), activation='relu', padding='same'))
    model.add(MaxPooling2D(pool_size=(2, 1), strides=2))
    model.add(Conv2D(filters=40, kernel_size=(10, 20), activation='relu', padding='same'))
    model.add(MaxPooling2D(pool_size=(2, 1), strides=2))
    model.add(Conv2D(filters=80, kernel_size=(10, 40), activation='relu', padding='same'))
    ...
    model_filename_CP = 'DCNN1Grid.h5'
    ...
    model_filename = 'DCNN1.h5'                                                     # Model File Name
    ...
    plt.ylabel('Loss')
    plt.legend()
    LossEpoch_name='DCNN1LossEpoch.png'                                      # LossEpoch file name
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    AccEpoch_name='DCNN1AccEpoch.png'                                      # AccEpoch file name
