...
def display_metrics(y_true, y_pred):
    ...
    disp.plot()
    conf_name='YANGConfusion2.png'                                      # Conf file name
...
def FCN(fileSeizure, fileNoSeizure):
    ...
    from tensorflow.keras.layers import ConvLSTM2D, Conv2D, MaxPooling2D, Flatten, Dense, BatchNormalization, Dropout
    ...
    for ix in indexes:
        df_Seizure = df_Seizure.drop(ix, inplace=False, axis=0)
        df_NoSeizure = df_NoSeizure.drop(ix, inplace=False, axis=0)
    ...
    # Estandarizar
    scaler_name='YANGScaler2.pkl'                               # Scaler file name 
    sc = StandardScaler()
    ...
    model.add(tf.keras.layers.Reshape((256, 18, 5, 1), input_shape=(23040,)))
    model.add(ConvLSTM2D(filters=16, kernel_size=(3, 19), strides=(1, 2), padding='same', return_sequences=True))
    model.add(ConvLSTM2D(filters=32, kernel_size=(3, 3), strides=(1, 2), padding='same', return_sequences=True))
    model.add(ConvLSTM2D(filters=64, kernel_size=(3, 3), strides=(1, 2), padding='same', return_sequences=False))
    model.add(Flatten())
    ...
    model_filename_CP = 'YANGGrid2.h5'
    ...
    model_filename = 'YANG2.h5'                                                     # Model File Name
    ...
    plt.ylabel('Loss')
    plt.legend()
    LossEpoch_name='YANGLossEpoch2.png'                                      # LossEpoch file name
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    AccEpoch_name='YANGAccEpoch2.png'                                      # AccEpoch file name
